...
import { Download, TrendingUp, Scale, Star, Activity, ShieldCheck, DollarSign, PieChart as PieIcon, Layers, BarChart3, ChevronRight, Briefcase } from 'lucide-react';
import { PieChart, Pie, Cell, ResponsiveContainer, Tooltip, BarChart, Bar, XAxis, YAxis, CartesianGrid, Legend } from 'recharts';
// Fixed: Removed non-existent FISCAL_YEARS from constants import
...
const Reports: React.FC = () => {
  ...
  const financialStats = useMemo(() => {
    ...
    // Provisión de impuestos estimada (Self-Assessment UK)
    const taxProvision = netProfit > 12570 ? (netProfit - 12570) * 0.20 : 0;

    const channelData = [
      { name: 'Snacks B2B', value: wholesaleIncome },
      { name: 'Snacks B2C', value: retailSnackIncome },
      { name: 'Servicios', value: retailServiceIncome },
    ];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
         ...
         <div className="bg-white p-12 rounded-[4rem] shadow-xl border border-slate-50 space-y-10">
            ...
            <section className="space-y-4">
               <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em] mb-4">INGRESOS POR CANAL</p>
            ...
            <section className="space-y-4 pt-6 border-t border-slate-100">
               <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em] mb-4">EGRESOS Y CARGAS</p>
         ...
         {/* GRÁFICO DE RENDIMIENTO POR CATEGORÍA/SABOR */}
         <div className="bg-white p-12 rounded-[4rem] flex flex-col justify-between shadow-xl border border-slate-50 relative overflow-hidden">
            <div className="absolute -top-10 -right-10 w-80 h-80 bg-[#20B2AA]/5 rounded-full blur-3xl"></div>
            <div className="relative z-10">
               ...
               <p className="text-slate-500 text-sm font-medium leading-relaxed max-w-sm">Auditoría de los 5 productos (sabores) que más contribuyen al flujo de caja anual.</p>
            </div>
            ...
            <div className="bg-slate-50 p-8 rounded-3xl border border-slate-100 flex items-center justify-between">
               <div className="flex items-center gap-4">
                  ...
                  <div>
                     ...
                     <p className="text-[9px] font-bold text-slate-400 uppercase mt-1">Comparativa de líneas snacks/servicios</p>
                  </div>
               </div>
               {/* Fixed the reference error for 'navigate' by providing it via useNavigate hook above */}
      ...
      <div className="bg-white p-12 rounded-[4.5rem] shadow-2xl border border-slate-100">
         <div className="flex justify-between items-center mb-12">
            <h3 className="text-3xl font-black text-slate-900 uppercase tracking-tighter flex items-center gap-4">
              <Activity className="text-[#FF6B9D]"/> AUDITORÍA DE SERVICIOS (DOG CARE)
            </h3>
            <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.4em]">Detalle B2C Retail</span>
         </div>
            ...
            {SERVICES.map((service, idx) => (
              <div key={idx} className="p-8 bg-slate-50 rounded-[2.5rem] border border-slate-100 hover:border-[#FF6B9D] transition-all group">
                 ...
                 <div className="flex justify-between items-end mt-6 pt-6 border-t border-slate-200">
                    ...
                    <div className="text-right">
                       <p className="text-[9px] font-black text-[#FF6B9D] uppercase">Tarifa</p>
...
const ReportLine = ({ label, value, negative, bold }: any) => (
